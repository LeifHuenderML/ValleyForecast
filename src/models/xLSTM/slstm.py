...
class sLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, dropout=0.01):
    ...
    def forward(self, input, hidden=None):
        ...
        for t in range(seq_length):
            x = input[:,t,:]
            new_hidden_state = []
            for index, (lstm, dropout, f_gate, i_gate) in enumerate(zip(self.lstms, self.dropout_layers, self.exp_forget_gates, self.exp_input_gates)):
                if hidden[i][0] is None:
                    h,c = lstm(x)
                else:
                    h,c = lstm(x, (hidden[index][0], hidden[index][1]))
